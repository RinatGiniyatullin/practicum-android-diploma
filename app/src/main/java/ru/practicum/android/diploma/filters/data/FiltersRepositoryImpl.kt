...
class FiltersRepositoryImpl(
    ...
    override suspend fun getCountries(): Flow<ru.practicum.android.diploma.util.Resource<List<Country>>> =
        flow {
            val response = networkClient.getCountries(CountriesSearchRequest)
            when (response.resultCode) {
                ERROR -> {
                    emit(
                        ru.practicum.android.diploma.util.Resource.Error(
                ...
                SUCCESS -> {
                    with(response) {
                        val countriesList = response.resultCountries.map { mapVacancyFromDto(it) }
